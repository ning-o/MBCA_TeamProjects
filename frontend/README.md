...
## 7. 라우터 사용법
- '모듈형 라우터' 방식을 사용
- App.js를 직접 수정 금지: 모든 경로는 각자 도메인별 폴더에 있는 [도메인이름]Router.js에 추가.
- 도메인별 파일 경로: frontend/domains/각 도메인별 폴더/도메인이름Router.js

*새로운 화면 만들 때 순서:*
- 자기 도메인 폴더에 [화면이름]Screen.js 파일 만들기.
- 해당 폴더의 [도메인이름]Router.js 파일 열기. **[도메인이름]Router.js 파일은 이미 각자 폴더에 만들어져 있음**
- Stack.Screen을 추가하여 화면을 등록. **자세한 사용법 코드 참고**
- 주의: name 속성은 앱 전체에서 고유해야 함. 다른 도메인과 겹치지 않게 주의 (예: FridgeMain, SubsMain 등 도메인명을 앞에 붙이는 것을 권장)
