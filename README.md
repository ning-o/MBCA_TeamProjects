...
3. **Compliance & Ethics:**
   ...
   - 데이터 보안: 유저의 개인정보(비밀번호 등)는 암호화하여 처리하며, 불필요한 정보 수집을 지양한다.
5. **Strict Protocol Compliance:** "어떻게 만들었느냐"는 각자의 자율에 맡기되, "무엇을 전달하느냐"는 합의된 API 명세서와 코드 컨벤션을 엄격히 준수한다.
6. **Git Flow & Code Review**: `main` - `develop` - `feature` 브랜치 전략을 통해 단순 병합이 아닌 코드 리뷰와 충돌 해결 중심의 협업 진행한다.
